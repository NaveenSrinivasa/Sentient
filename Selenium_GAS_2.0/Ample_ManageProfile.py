...
def EditProfile(existing_profile_name=None, profile_json=None, save_key=None):
    ...
    if SelectProfileManage(existing_profile_name):
        with open(Global.testResourcePath + profile_json, 'r') as infile:
        ...
        if 'No change' in message or 'already exist' in message or 'error' in message:
...
def ValidateProfileFieldValues(input_file_path):
    ...
    """
    with open(Global.testResourcePath + input_file_path, 'rt') as f:
        reader = csv.reader(f)
...
def CompareDefault(profileFile, testfield, testfieldvalue):
    ...
    with open(Global.testResourcePath + profileFile,'r') as reqdnp3pointslist:
